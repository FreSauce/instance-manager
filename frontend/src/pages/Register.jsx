						...
						<MediaQuery query="(max-width: 600px)" styles={{ width: '100%' }}>
							<Box sx={{ width: '70%', backgroundColor: '#191c24', borderRadius: '4px', color: 'white' }} p={30} pt={15}>
								...
								<form onSubmit={form.onSubmit(handleSubmit)}>
									<Text mb={5} sx={{ fontWeight: '600', fontSize: '1.25rem' }}>New here?</Text>
									<Text mb={15} sx={{ fontWeight: '500', fontSize: '1.1rem' }}>Signing up is easy. It only takes a few steps</Text>
									<TextInput
										mb={13}
										withAsterisk
										label='Username'
									...
									/>
									<Checkbox mt={10} {...form.getInputProps('terms', { type: 'checkbox' })} label='I agree to sell my privacy' />
									<Button disabled={!form.isValid('terms')} fullWidth type='submit' mt={20} loading={loading}>
										<Text my={20} sx={{ fontWeight: '600', fontSize: '1rem' }}>Register</Text>
									</Button>
								</form>
								<Text mt={10} sx={{ fontWeight: '600', fontSize: '0.9rem' }}>Already have an account?{" "}
									<Link mt={10} style={{ textDecoration: 'none' }} to={'/login'}>
										<Text variant='link' sx={{ display: 'inline' }}>Sign In</Text>
									</Link>
								</Text>
							</Box>
						</MediaQuery>
